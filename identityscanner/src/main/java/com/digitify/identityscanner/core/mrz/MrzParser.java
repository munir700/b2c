 ...
 */
package com.digitify.identityscanner.core.mrz;
...
public class MrzParser {
    private static final String TAG = MrzParser.class.getSimpleName();
     ...
     */
    public boolean checkDigit(int col, int row, MrzRange strRange, String fieldName) throws RuntimeException {
     ...
     * @return true if check digit is valid, false if not
     */
    public boolean checkDigit(int col, int row, String str, String fieldName) throws RuntimeException {
     ...
     */
    public MrzDate parseDate(MrzRange range) throws IllegalArgumentException {
        if (range.length() != 6) {
        ...
        try {
            ...
            year = -1;
            Log.d(TAG, "Failed to parse MRZ date year " + rawValue(range) + ": " + ex + mrz + r);
        }
        if (year < 0 || year > 99) {
            Log.d(TAG, "Invalid year value " + year + ": must be 0..99");
        }
        ...
        try {
            ...
            month = -1;
            Log.d(TAG, "Failed to parse MRZ date month " + rawValue(range) + ": " + ex + mrz + r);
        }
        if (month < 1 || month > 12) {
            Log.d(TAG, "Invalid month value " + month + ": must be 1..12");
        }
        ...
        if (day < 1 || day > 31) {
            Log.d(TAG, "Invalid day value " + day + ": must be 1..31");
        }
     ...
     */
    public Gender parseSex(int col, int row) throws RuntimeException {
        return Gender.fromMrz(rows[row].charAt(col));
    }
    ...
    private static int getCharacterValue(char c) throws RuntimeException {
        if (c == FILLER) {
            return 0;
        }
        if (c >= '0' && c <= '9') {
     ...
     */
    public static int computeCheckDigit(String str) throws RuntimeException {
        int result = 0;
        for (int i = 0; i < str.length(); i++) {
     ...
     * @return record class.
     */
    public static MrzRecord parse(String mrz) throws RuntimeException {
