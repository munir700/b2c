...
object DateUtils {
    ...
    val LOCAL: Locale = Locale.ENGLISH
    ...
    private fun normaliseDate(day: Int, month: Int, year: Int): Date {
        ...
        val format = SimpleDateFormat("dd-mm-yy", LOCAL)
    ...
    fun reformatStringDate(
        ...
        var result = ""
        val formatter = SimpleDateFormat(outFormatter, LOCAL)
        try {
    ...
    fun dateToString(
        ...
        return try {
            SimpleDateFormat(format, LOCAL).format(date)
...
//            val symbols = DateFormatSymbols(LOCAL)
...
//            formatter.dateFormatSymbols = symbols
    ...
    fun stringToDate(dateStr: String, format: String): Date? {
        ...
        try {
            ...
            formatter.timeZone = TIME_ZONE_Default
            val newDate = SimpleDateFormat(format, LOCAL).format(d)
    ...
    fun reformatLocalDate(
        ...
        return try {
            val inFormatter = SimpleDateFormat(inputFormatter, LOCAL)
            val outFormatter = SimpleDateFormat(outputFormatter, LOCAL)
    ...
    fun reformatToLocalString(
        ...
        return try {
            SimpleDateFormat(outputFormatter, LOCAL).format(date)
            val outFormatter = SimpleDateFormat(outputFormatter, LOCAL)
    ...
    fun stringToDateLeanPlum(dateStr: String): Date? {
        var d: Date? = null
        val formatter = SimpleDateFormat(LEAN_PLUM_FORMAT, LOCAL)
    ...
    fun getCurrentDateWithFormat(formal: String): String {
        val sdf = SimpleDateFormat(formal, LOCAL)
