...
fun Content?.getTransactionTypeIcon(): Int {
    this?.let { transaction ->
        if (TransactionStatus.FAILED.name == transaction.status || transaction.status == TransactionStatus.CANCELLED.name || TransactionProductCode.MANUAL_ADJUSTMENT.pCode == transaction.productCode) return android.R.color.transparent
