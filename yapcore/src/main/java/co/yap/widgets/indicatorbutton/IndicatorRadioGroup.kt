...
class IndicatorRadioGroup @JvmOverloads constructor(
    ...
    private var childOnCheckedChangeListener: CheckableIndicatorButton.OnCheckedChangeListener? =
        null
    ...
    override fun addView(child: View?, index: Int, params: ViewGroup.LayoutParams?) {
        if (child is RadioIndicatorButton) {
            if (child.isChecked) {
              ...
              //  protectFromCheckedChange = true
               // performClick()
                    //setCheckedStateForView(child[0].id, false)
                //protectFromCheckedChange = false

               // setCheckedId(child[0].id)
//                setCheckedStateForView(child[0].id, false)
            }
