...
class CCPCountryGroup private constructor(defaultNameCode:String, areaCodeLength:Int, nameCodeToAreaCodesMap:HashMap<String, String>) {
    ...
    fun getCountryForAreaCode(context:Context, language:CountryCodePicker.Language, areaCode:String):CCPCountry {
        ...
        for (entry in nameCodeToAreaCodesMap.entries)
        {
            if (entry.value.contains(areaCode))
            {
                nameCode = entry.key
            }
        }
