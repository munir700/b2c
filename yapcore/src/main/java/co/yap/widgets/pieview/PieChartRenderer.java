...
public class PieChartRenderer extends DataRenderer {
    ...
    public PieChartRenderer(PieChart chart, ChartAnimator animator,
        ...
        mCenterTextPaint.setTextSize(UtilsPieView.convertDpToPixel(12f));

        mValuePaint.setTextSize(UtilsPieView.convertDpToPixel(13f));
        ...
        mEntryLabelsPaint.setTextSize(UtilsPieView.convertDpToPixel(13f));
    ...
    protected float calculateMinimumRadiusForSpacedSlice(
        ...
        float arcEndPointX = center.x + radius * (float) Math.cos((startAngle + sweepAngle) * UtilsPieView.FDEG2RAD);
        float arcEndPointY = center.y + radius * (float) Math.sin((startAngle + sweepAngle) * UtilsPieView.FDEG2RAD);

        // Middle point on the arc
        float arcMidPointX = center.x + radius * (float) Math.cos(angleMiddle * UtilsPieView.FDEG2RAD);
        float arcMidPointY = center.y + radius * (float) Math.sin(angleMiddle * UtilsPieView.FDEG2RAD);
        ...
        float containedTriangleHeight = (float) (basePointsDistance / 2.0 *
                Math.tan((180.0 - angle) / 2.0 * UtilsPieView.DEG2RAD));
    ...
    protected void drawDataSet(Canvas c, IPieDataSet dataSet) {
        ...
        for (int j = 0; j < entryCount; j++) {
            ...
            if ((Math.abs(dataSet.getEntryForIndex(j).getY()) > UtilsPieView.FLOAT_EPSILON)) {
                visibleAngleCount++;
            }
        }
        ...
        for (int j = 0; j < entryCount; j++) {
            ...
            // draw only if the value is greater than zero
            if (!(Math.abs(e.getY()) > UtilsPieView.FLOAT_EPSILON)) {
            ...
            final float sliceSpaceAngleOuter = visibleAngleCount == 1 ?
                    0.f :
                    sliceSpace / (UtilsPieView.FDEG2RAD * radius);
            ...
            if (drawRoundedSlices) {
                float x = center.x + (radius - roundedRadius) * (float) Math.cos(startAngleOuter * UtilsPieView.FDEG2RAD);
                float y = center.y + (radius - roundedRadius) * (float) Math.sin(startAngleOuter * UtilsPieView.FDEG2RAD);
                roundedCircleBox.set(x - roundedRadius, y - roundedRadius, x + roundedRadius, y + roundedRadius);
            }

            float arcStartPointX = center.x + radius * (float) Math.cos(startAngleOuter * UtilsPieView.FDEG2RAD);
            float arcStartPointY = center.y + radius * (float) Math.sin(startAngleOuter * UtilsPieView.FDEG2RAD);

            if (sweepAngleOuter >= 360.f && sweepAngleOuter % 360f <= UtilsPieView.FLOAT_EPSILON) {
            ...
            if (drawInnerArc && (innerRadius > 0.f || accountForSliceSpacing)) {
                ...
                final float sliceSpaceAngleInner = visibleAngleCount == 1 || innerRadius == 0.f ?
                        0.f :
                        sliceSpace / (UtilsPieView.FDEG2RAD * innerRadius);
                ...
                if (sweepAngleOuter >= 360.f && sweepAngleOuter % 360f <= UtilsPieView.FLOAT_EPSILON) {
                    ...
                    if (drawRoundedSlices) {
                        float x = center.x + (radius - roundedRadius) * (float) Math.cos(endAngleInner * UtilsPieView.FDEG2RAD);
                        float y = center.y + (radius - roundedRadius) * (float) Math.sin(endAngleInner * UtilsPieView.FDEG2RAD);
                        roundedCircleBox.set(x - roundedRadius, y - roundedRadius, x + roundedRadius, y + roundedRadius);
                        mPathBuffer.arcTo(roundedCircleBox, endAngleInner, 180);
                    } else
                        mPathBuffer.lineTo(
                                center.x + innerRadius * (float) Math.cos(endAngleInner * UtilsPieView.FDEG2RAD),
                                center.y + innerRadius * (float) Math.sin(endAngleInner * UtilsPieView.FDEG2RAD));
                ...
                if (sweepAngleOuter % 360f > UtilsPieView.FLOAT_EPSILON) {
                    if (accountForSliceSpacing) {
                        ...
                        float arcEndPointX = center.x +
                                sliceSpaceOffset * (float) Math.cos(angleMiddle * UtilsPieView.FDEG2RAD);
                        float arcEndPointY = center.y +
                                sliceSpaceOffset * (float) Math.sin(angleMiddle * UtilsPieView.FDEG2RAD);
    ...
    public void drawValues(Canvas c) {
        ...
        float offset = UtilsPieView.convertDpToPixel(5.f);
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            float lineHeight = UtilsPieView.calcTextHeight(mValuePaint, "Q")
                    + UtilsPieView.convertDpToPixel(4f);
            ...
            mValueLinePaint.setStrokeWidth(UtilsPieView.convertDpToPixel(dataSet.getValueLineWidth()));

            final float sliceSpace = getSliceSpace(dataSet);

            MPPointF iconsOffset = MPPointF.getInstance(dataSet.getIconsOffset());
            iconsOffset.x = UtilsPieView.convertDpToPixel(iconsOffset.x);
            iconsOffset.y = UtilsPieView.convertDpToPixel(iconsOffset.y);
            ...
            for (int j = 0; j < entryCount; j++) {
                ...
                final float sliceSpaceMiddleAngle = sliceSpace / (UtilsPieView.FDEG2RAD * labelRadius);
                ...
                final float sliceXBase = (float) Math.cos(transformedAngle * UtilsPieView.FDEG2RAD);
                final float sliceYBase = (float) Math.sin(transformedAngle * UtilsPieView.FDEG2RAD);
                ...
                if (drawXOutside || drawYOutside) {
                    ...
                    final float polyline2Width = dataSet.isValueLineVariableLength()
                            ...
                            transformedAngle * UtilsPieView.FDEG2RAD))
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (int i = 0; i < indices.length; i++) {
            ...
            for (int j = 0; j < entryCount; j++) {
                // draw only if the value is greater than zero
                if ((Math.abs(set.getEntryForIndex(j).getY()) > UtilsPieView.FLOAT_EPSILON)) {
                    visibleAngleCount++;
                }
            }
            ...
            final float sliceSpaceAngleShifted = visibleAngleCount == 1 ?
                    0.f :
                    sliceSpace / (UtilsPieView.FDEG2RAD * highlightedRadius);
            ...
            if (sweepAngleOuter >= 360.f && sweepAngleOuter % 360f <= UtilsPieView.FLOAT_EPSILON) {
                ...
                mPathBuffer.moveTo(
                        center.x + highlightedRadius * (float) Math.cos(startAngleShifted * UtilsPieView.FDEG2RAD),
                        center.y + highlightedRadius * (float) Math.sin(startAngleShifted * UtilsPieView.FDEG2RAD));
            ...
            if (accountForSliceSpacing) {
                sliceSpaceRadius =
                        calculateMinimumRadiusForSpacedSlice(
                                center, radius,
                                sliceAngle * phaseY,
                                center.x + radius * (float) Math.cos(startAngleOuter * UtilsPieView.FDEG2RAD),
                                center.y + radius * (float) Math.sin(startAngleOuter * UtilsPieView.FDEG2RAD),
                                startAngleOuter,
                                sweepAngleOuter);
            }
                ...
                if (sweepAngleOuter % 360f > UtilsPieView.FLOAT_EPSILON) {
                    ...
                    if (accountForSliceSpacing) {
                        ...
                        final float arcEndPointX = center.x +
                                sliceSpaceRadius * (float) Math.cos(angleMiddle * UtilsPieView.FDEG2RAD);
                        final float arcEndPointY = center.y +
                                sliceSpaceRadius * (float) Math.sin(angleMiddle * UtilsPieView.FDEG2RAD);
