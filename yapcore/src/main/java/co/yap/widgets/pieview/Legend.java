...
public class Legend extends ComponentBase {
    ...
    public Legend() {
        ...
        this.mTextSize = UtilsPieView.convertDpToPixel(10f);
        this.mXOffset = UtilsPieView.convertDpToPixel(5f);
        this.mYOffset = UtilsPieView.convertDpToPixel(3f); // 2
    }
    ...
    public float getMaximumEntryWidth(Paint p) {
        ...
        float formToTextSpace = UtilsPieView.convertDpToPixel(mFormToTextSpace);

        for (LegendEntry entry : mEntries) {
            final float formSize = UtilsPieView.convertDpToPixel(
            ...
            float length = (float) UtilsPieView.calcTextWidth(p, label);
    ...
    public float getMaximumEntryHeight(Paint p) {
        ...
        for (LegendEntry entry : mEntries) {
            ...
            float length = (float) UtilsPieView.calcTextHeight(p, label);
    ...
    public void calculateDimensions(Paint labelpaint, ViewPortHandler viewPortHandler) {
        ...
        float defaultFormSize = UtilsPieView.convertDpToPixel(mFormSize);
        float stackSpace = UtilsPieView.convertDpToPixel(mStackSpace);
        float formToTextSpace = UtilsPieView.convertDpToPixel(mFormToTextSpace);
        float xEntrySpace = UtilsPieView.convertDpToPixel(mXEntrySpace);
        float yEntrySpace = UtilsPieView.convertDpToPixel(mYEntrySpace);
        ...
        switch (mOrientation) {
            case VERTICAL: {
                ...
                float labelLineHeight = UtilsPieView.getLineHeight(labelpaint);
                ...
                for (int i = 0; i < entryCount; i++) {
                    ...
                    float formSize = Float.isNaN(e.formSize)
                            ? defaultFormSize
                            : UtilsPieView.convertDpToPixel(e.formSize);
                    ...
                    if (label != null) {
                        ...
                        width += UtilsPieView.calcTextWidth(labelpaint, label);
            ...
            case HORIZONTAL: {
                ...
                float labelLineHeight = UtilsPieView.getLineHeight(labelpaint);
                float labelLineSpacing = UtilsPieView.getLineSpacing(labelpaint) + yEntrySpace;
                ...
                for (int i = 0; i < entryCount; i++) {
                    ...
                    if (label != null) {
                        ...
                        mCalculatedLabelSizes.add(UtilsPieView.calcTextSize(labelpaint, label));
