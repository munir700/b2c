...
public class ShadowBottomNavigationView extends BottomNavigationView {
    ...
    public ShadowBottomNavigationView(Context context) {
        super(context);
        setBackground(gd);
    }

    public ShadowBottomNavigationView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }
    ...
    private void init(AttributeSet set) {
        TypedArray ta = getContext().obtainStyledAttributes(set, R.styleable.ShadowRelativeLayout);
        boolean shadow = ta.getBoolean(R.styleable.ShadowRelativeLayout_shadow, false);
        String shadowColor = ta.getString(R.styleable.ShadowRelativeLayout_shadowColor);
        if (shadowColor == null) {
            shadowColor = "000000";
        }
        transferClick = ta.getBoolean(R.styleable.ShadowRelativeLayout_clickTransferable, false);
        selfClickable = ta.getBoolean(R.styleable.ShadowRelativeLayout_selfClickable, true);
        gradientAngle = ta.getString(R.styleable.ShadowRelativeLayout_gradientAngle);
        clickAfterAnimation = ta.getBoolean(R.styleable.ShadowRelativeLayout_clickAfterAnimation, true);
        if (gradientAngle == null) gradientAngle = "TOP_BOTTOM";
        int gradientCenterColor = ta.getColor(R.styleable.ShadowRelativeLayout_gradientCenterColor, 0);
        int gradientEndColor = ta.getInteger(R.styleable.ShadowRelativeLayout_gradientEndColor, 0);
        int gradientStartColor = ta.getInt(R.styleable.ShadowRelativeLayout_gradientStartColor, 0);
        colors[0] = gradientStartColor;
        colors[1] = gradientCenterColor;
        colors[2] = gradientEndColor;
        gradientType = ta.getString(R.styleable.ShadowRelativeLayout_gradientType);
        if (gradientType == null) {
            gradientType = "linear";
        }
        int spread = ta.getInt(R.styleable.ShadowRelativeLayout_shadowSpread, 1);
        int shadowAlpha = ta.getInt(R.styleable.ShadowRelativeLayout_shadowAlpha, 255);
        animationDuration = ta.getInteger(R.styleable.ShadowRelativeLayout_animationDuration, 2000);
        toXScale = ta.getFloat(R.styleable.ShadowRelativeLayout_toXScale, 1.0f);
        fromXScale = ta.getFloat(R.styleable.ShadowRelativeLayout_fromXScale, .3f);
        toYScale = ta.getFloat(R.styleable.ShadowRelativeLayout_toYScale, 1.0f);
        fromYScale = ta.getFloat(R.styleable.ShadowRelativeLayout_fromYScale, .3f);
        pivotX = ta.getFloat(R.styleable.ShadowRelativeLayout_pX, -1);
        pivotY = ta.getFloat(R.styleable.ShadowRelativeLayout_pY, -1);
        animate = ta.getBoolean(R.styleable.ShadowRelativeLayout_animate, false);
        rad = ta.getDimension(R.styleable.ShadowRelativeLayout_radius, 0);
        topRightRadius = ta.getDimension(R.styleable.ShadowRelativeLayout_topRightRadius, 0);
        topLeftRadius = ta.getDimension(R.styleable.ShadowRelativeLayout_topLeftRadius, 0);
        bottomRightRadius = ta.getDimension(R.styleable.ShadowRelativeLayout_bottomRightRadius, 0);
        bottomLeftRadius = ta.getDimension(R.styleable.ShadowRelativeLayout_bottomLeftRadius, 0);
        color = ta.getColor(R.styleable.ShadowRelativeLayout_color, android.R.attr.colorBackground);
        onclickColor = ta.getColor(R.styleable.ShadowRelativeLayout_onclickColor, -1);
        interpolate = ta.getBoolean(R.styleable.ShadowRelativeLayout_interpolate, false);
        amplitude = ta.getFloat(R.styleable.ShadowRelativeLayout_amplitude, 1.0f);
        frequency = ta.getFloat(R.styleable.ShadowRelativeLayout_frequency, .3f);
        String sh = ta.getString(R.styleable.ShadowRelativeLayout_shape);
...
//                animation.setAnimationListener(ShadowBottomNavigationView.this);
//                if (interpolate) {
//                    Interpolator interpolator = new ShadowBottomNavigationView.DefaultInterpolator();
...
//        });
        ...
        String sPosition = ta.getString(R.styleable.ShadowRelativeLayout_shadowPosition);
        if (sPosition != null) {
            switch (sPosition) {
                case "top":
...
//        if (parent instanceof ShadowBottomNavigationView && transferClick) {
//            ShadowBottomNavigationView complexView = (ShadowBottomNavigationView) parent;
