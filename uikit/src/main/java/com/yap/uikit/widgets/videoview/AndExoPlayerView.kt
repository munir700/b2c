package com.yap.uikit.widgets.videoview
...
class AndExoPlayerView : LinearLayout {
    ...
    constructor(
        ...
        typedArray = context.theme.obtainStyledAttributes(
            attrs,
            com.yap.uikit.R.styleable.AndExoPlayerView,
            0, 0
        )
        initializeView(context)
    }
    ...
    private fun initializeView(context: Context) {
        ...
        val view =
            inflater.inflate(com.yap.uikit.R.layout.layout_video_player_base, this, true)
        playerView = view.findViewById(com.yap.uikit.R.id.simpleExoPlayerView)
        componentListener = ComponentListener()
        if (typedArray != null) {
            if (typedArray!!.hasValue(com.yap.uikit.R.styleable.AndExoPlayerView_andexo_resize_mode)) {
                val resizeMode = typedArray!!.getInteger(
                    com.yap.uikit.R.styleable.AndExoPlayerView_andexo_resize_mode,
                    EnumResizeMode.FILL.value
                )
                setResizeMode(EnumResizeMode.get(resizeMode))
            }
            if (typedArray!!.hasValue(com.yap.uikit.R.styleable.AndExoPlayerView_andexo_aspect_ratio)) {
                val aspectRatio = typedArray!!.getInteger(
                    com.yap.uikit.R.styleable.AndExoPlayerView_andexo_aspect_ratio,
                    EnumAspectRatio.ASPECT_16_9.value
                )
                setAspectRatio(EnumAspectRatio.get(aspectRatio))
            }
            if (typedArray!!.hasValue(com.yap.uikit.R.styleable.AndExoPlayerView_andexo_play_when_ready)) {
                setPlayWhenReady(
                    typedArray!!.getBoolean(
                        com.yap.uikit.R.styleable.AndExoPlayerView_andexo_play_when_ready,
                        false
                    )
                )
            }
    ...
    fun setAspectRatio(aspectRatio: EnumAspectRatio) {
        ...
        when (aspectRatio) {
            ...
            EnumAspectRatio.ASPECT_MP3 -> {
                ...
                val mp3Height =
                    context.resources.getDimensionPixelSize(com.yap.uikit.R.dimen._40sdp)
            ...
            EnumAspectRatio.UNDEFINE -> {
                val baseHeight =
                    resources.getDimension(com.yap.uikit.R.dimen._220sdp).toInt()
                playerView?.layoutParams = FrameLayout.LayoutParams(
                    ViewGroup.LayoutParams.MATCH_PARENT,
                    baseHeight
                )
