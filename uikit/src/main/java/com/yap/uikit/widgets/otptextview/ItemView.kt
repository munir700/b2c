...
class ItemView : FrameLayout {
    ...
    private fun generateViews(styles: TypedArray) {
        val defaultHeight = context.dip2px(DEFAULT_BAR_HEIGHT).toFloat()
        val defaultOtpTextSize = context.dip2px(DEFAULT_OTP_TEXT_SIZE).toFloat()
        val textColor = styles.getColor(
            R.styleable.OtpTextView_android_textColor, ResourcesCompat.getColor(
                context.resources,
        ...
        val barMargin = styles.getDimension(R.styleable.OtpTextView_bar_margin, context.dip2px(0f))
        ...
        val boxBackgroundColor = styles.getResourceId(
            R.styleable.OtpTextView_otp_box_background, ResourcesCompat.getColor(
                context.resources,
        ...
        barActiveColor = styles.getColor(
            R.styleable.OtpTextView_bar_active_color, ResourcesCompat.getColor(
                context.resources,
                android.R.color.black, null
            )
        )
        barInactiveColor = styles.getColor(
            R.styleable.OtpTextView_bar_inactive_color, ResourcesCompat.getColor(
                context.resources,
                R.color.otp_inactive_color_default, null
            )
        )
        barErrorColor = styles.getColor(
            R.styleable.OtpTextView_bar_error_color, ResourcesCompat.getColor(
                context.resources,
                R.color.otp_error_color_default, null
            )
        )
        barSuccessColor = styles.getColor(
            R.styleable.OtpTextView_bar_success_color, ResourcesCompat.getColor(
                context.resources,
                android.R.color.black, null
            )
        )
